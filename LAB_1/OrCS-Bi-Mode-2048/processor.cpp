...
void adjust(Predictor * predict, int adjustment, uint64_t address, bool hit){
	...
	if(hit)
		predict->global_history = ((predict->global_history << 1) + 1) & 2047;
	else
		predict->global_history = (predict->global_history << 1) & 2047;
}
...
void processor_t::allocate() {
	...
	predictor->direction_predictors = ( char *)malloc(4096 * sizeof(char));
	predictor->choice_predictor = (char *)malloc(2048 * sizeof(char));

	memset(predictor->direction_predictors, 0, 2048 * sizeof(char));
	memset(predictor->direction_predictors + 2048, 3, 1024 * sizeof(char));
	memset(predictor->choice_predictor, 0, 2048 * sizeof(char));
